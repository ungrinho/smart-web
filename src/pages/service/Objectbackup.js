                        ...
                        <Box sx={{ margin: 1 }}>
                            ...
                            <Table size="small" aria-label="purchases">
                                ...
                                <TableBody>
                                    {(() => {
                                        const rows = [];
                                        for (const category in row.categories) {
                                            rows.push(
                                                <TableRow key={category} onClick={() => onImageClick(row.images[category][0])}>
                                                    <ClickTableCell component="th" scope="row">
                                                        {category}
                                                    </ClickTableCell>
                                                    <TableCell align="right">{row.categories[category]}</TableCell>
                                                </TableRow>
                                            );
                                        }
                                        return rows;
    ...
    const handleImageClick = async (fileName) => {
        try {
            if (fileName.includes('ripe')) {
                setSelectedImage('/ripe-tomato.jpg');
            } else if (fileName.includes('unripe')) {
                setSelectedImage('/unripe-tomato.jpg');
            } else if (fileName.includes('green')) {
                setSelectedImage('/green-tomato.jpg');
            } else {
                const url = await getDownloadURL(ref(storage, fileName));
                setSelectedImage(url);
            }
